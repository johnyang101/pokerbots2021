...
Implement win percent.
Calculate strength of hands post-flop.
Reform betting logic.
Bluffing
'''
...
class Player(Bot):
    ...
    def __init__(self):
        ...
        self.winpercent = {'AA': 85.255, 'KK': 82.39, 'QQ': 79.935, 'JJ': 77.485, 'TT': 75.02, '99': 72.09, '88': 69.175, '77': 66.24, 'KAs': 66.61, 'QAs': 65.8, 'JAs': 65.025, 'KA': 64.865, 'TAs': 64.25, '66': 63.17, 'QA': 64.005, 'JA': 63.16, 'QKs': 63.065, 'TA': 62.345, 'JKs': 62.26, '9As': 62.425, 'TKs': 61.49, '8As': 61.615, '55': 60.34, 'QK': 61.115, 'JK': 60.265,
    ...
    def calculate_strength_postflop(self, hole, iters, community):
        ...
        for i in range(NUM_BOARDS):
            if AssignAction in legal_actions[i]: #allocate cards
                ...
                if street < 3: #means pre-flop
                    ...
                    if not BIG_BLIND:  
                        if strength>self.preflop_value_bet_lp:
                            raise_amount = int(round(my_pips[i] + board_cont_cost + (strength + self.initial_hole_lp) * (pot_total + board_cont_cost) + min(12, round((random_factor * (i + 1))/self.decay_factor_lp))))                        
                    ...
                    strength = self.calculate_strength_postflop(my_cards[i], _MONTE_CARLO_ITERS, board_cards[i])
                    random_factor = random.random()
                    doggo = round(my_pips[i] + board_cont_cost + (strength + self.initial_hole_lp) * (pot_total + board_cont_cost) + min(12, round((random_factor * (i + 1))/self.decay_factor_lp)))
                    print(doggo)
                    print('initial hole lp ' + str(self.initial_hole_lp)) 
